...
async function albumUrl(path) {
  const key = 'yuva:album-url:' + path;
  try {
    const c = JSON.parse(localStorage.getItem(key) || 'null');
    if (c && c.exp > Date.now()) return c.url;
  } catch { /* depo kapalı — önemsiz */ }
  const r = must(await sb().storage.from('album').createSignedUrl(path, URL_OMUR));
...
export default function makeSupabaseRepo() {
  return {
    ...
    summary: {
      ...
      async trend(period, months = 6) { return must(await sb().rpc('month_trend', { hid: hid(), p_period: period, p_months: months })); },
