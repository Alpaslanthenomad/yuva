...
export default function makeSupabaseRepo() {
  return {
    ...
    async init() {
      ...
      const profile = must(await sb().from('profiles').select('*').eq('user_id', user.id).single());
      let household = null;
      if (profile.default_household_id) {
        household = must(await sb().from('households').select('*').eq('id', profile.default_household_id).single());
      } else {
        const mine = must(await sb().from('household_members').select('household_id').eq('user_id', user.id).limit(1));
        if (mine[0]) household = must(await sb().from('households').select('*').eq('id', mine[0].household_id).single());
      }
      if (!household) return { household: null, members: [], me: null, user };
      const members = must(await sb().from('household_members').select('*').eq('household_id', household.id).eq('is_active', true));
      ctx = { household, members, me: members.find((m) => m.user_id === user.id) || null };
      return { ...ctx, user };
    },
    household: {
    ...
    budgets: {
      ...
      async set(b) { return must(await sb().from('budgets').upsert({ ...b, household_id: hid() }, { onConflict: 'household_id,period,category_id' }).select().single()); },
    },
    recurring: {
    ...
    tasks: {
      ...
      async toggle(id) {
        const cur = must(await sb().from('tasks').select('is_done').eq('id', id).single());
        return must(await sb().from('tasks').update({ is_done: !cur.is_done, done_at: cur.is_done ? null : new Date().toISOString() }).eq('id', id).select().single());
      },
    },
    shopping: {
    ...
    auth: {
      async signInWithEmail(email) {
        must(await sb().auth.signInWithOtp({ email, options: { emailRedirectTo: typeof window !== 'undefined' ? window.location.origin : undefined } }));
      },
