...
export default function makeSupabaseRepo() {
  return {
    ...
    suggest: {
      // Mağaza → en sık kategori (0035). Önbellek yok: form her açılışta
      // taze okur, kategorisi düzeltilen harcama hemen öğrenmeye yansır.
      async merchants() {
        try { return must(await sb().rpc('merchant_categories', { hid: hid(), p_limit: 300 })) || []; }
        catch { return []; }
      },
      async expenses(date) {
