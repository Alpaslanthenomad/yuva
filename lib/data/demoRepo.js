...
import { gununSirasi } from '../album.js';
import { sonrakiVade } from '../expenseCatalog.js';
import { magazaOgren } from '../hizliGiris.js';
import { harcamaOnerileri, bitmisOlabilir } from '../oneriler.js';
...
/**
 * 0028'in aynası: tutarı olan plan kalemi "ödendi" ise karşılığında bir
 * harcama vardır; işaret kalkınca harcama silinir; tutar değişince güncellenir.
 */
function masrafEsitle(db, item) {
  const odenmis = item.is_done && Number(item.amount) > 0;
  const mevcut = item.transaction_id ? db.transactions.find((t) => t.id === item.transaction_id) : null;
  if (!odenmis) {
    if (mevcut) db.transactions = db.transactions.filter((t) => t.id !== mevcut.id);
    item.transaction_id = null;
    return;
  }
  const acc = db.accounts.find((a) => a.id === item.account_id) || db.accounts[0];
  const ccy = item.currency || acc?.currency || db.household.base_currency;
  if (mevcut) {
    Object.assign(mevcut, { amount: Number(item.amount), merchant: item.title, currency: ccy,
      category_id: item.category_id || mevcut.category_id });
    mevcut.amount_base = Math.round(toBase(db, mevcut.amount, ccy) * 100) / 100;
    return;
  }
  const row = {
    id: uid('t'), household_id: db.household.id, kind: 'expense', account_id: acc?.id, amount: Number(item.amount),
    currency: ccy, occurred_on: item.on_date || today(), category_id: item.category_id || null,
    merchant: item.title, plan_id: item.plan_id, paid_by_member_id: db.me?.id, for_member_id: null, tags: [],
  };
  row.fx_rate = toBase(db, 1, ccy);
  row.amount_base = Math.round(toBase(db, row.amount, ccy) * 100) / 100;
  ...
  return {
    ...
    transactions: {
      async list({ period, limit, planId } = {}) {
        ...
        // 0029'un aynası: başkasının kişisel harcaması yalnızca maskeli görünür.
        rows = rows.map((t) => (t.for_member_id && t.for_member_id !== db.me?.id
          ? { id: t.id, kind: t.kind, account_id: t.account_id, amount: t.amount, currency: t.currency,
              amount_base: t.amount_base, occurred_on: t.occurred_on, for_member_id: t.for_member_id, masked: true }
          : t));
        if (planId) rows = rows.filter((t) => !t.masked);
      ...
      async remove(id) {
        const db = load();
        const eski = db.transactions.find((x) => x.id === id)?.receipt_path;
        if (eski && db.fisler) delete db.fisler[eski];
        db.transactions = db.transactions.filter((x) => x.id !== id);
        // Plan masrafından geldiyse kalem "ödenmedi"ye döner (0028).
        for (const i of db.plan_items) if (i.transaction_id === id) { i.transaction_id = null; i.is_done = false; }
        save(); return delay();
      },
    },
    budgets: {
      ...
      async set(b) {
        const db = load();
        // 0029: "bundan sonra böyle" — ileri aylardaki aynı kategori ayarı silinir.
        db.budgets = db.budgets.filter((x) => !((x.category_id || null) === (b.category_id || null) && x.period > b.period));
        const ex = db.budgets.find((x) => x.period === b.period && (x.category_id || null) === (b.category_id || null));
      ...
      async setPersonalLimit(amount) {
        const db = load(); db.personal_limit = Number(amount) > 0 ? Number(amount) : null; save(); return delay();
      },
      async remove(period, categoryId) {
        const db = load();
        const ayni = (b) => (b.category_id || null) === (categoryId || null);
        db.budgets = db.budgets.filter((b) => !(ayni(b) && b.period >= period));
        if (db.budgets.some((b) => ayni(b) && b.period < period)) {
          db.budgets.push({ id: uid('b'), household_id: db.household.id, rollover: false, period, category_id: categoryId || null, amount_base: 0 });
        }
        save(); return delay();
      },
    },
    recurring: {
      ...
      async remove(id) { const db = load(); db.recurring = db.recurring.filter((x) => x.id !== id); save(); return delay(); },
      // 0033'ün aynası: harcama yaz, vadeyi ilerlet.
      async pay(id, opts = {}) {
        const db = load(); const r = db.recurring.find((x) => x.id === id);
        if (!r) throw new Error('ödeme bulunamadı');
        const row = { id: uid('t'), household_id: db.household.id, kind: r.kind, account_id: opts.accountId || r.account_id || db.accounts[0]?.id,
          amount: opts.amount ?? r.amount, currency: r.currency, occurred_on: opts.date || today(), category_id: r.category_id,
          merchant: r.name, recurring_id: r.id, paid_by_member_id: db.me?.id, for_member_id: null, tags: [] };
        row.fx_rate = toBase(db, 1, row.currency);
        row.amount_base = Math.round(toBase(db, row.amount, row.currency) * 100) / 100;
        db.transactions.push(row);
        r.next_due_on = sonrakiVade(r.rrule, r.next_due_on);
        save(); return delay(row);
      },
      async skip(id) { const db = load(); const r = db.recurring.find((x) => x.id === id); r.next_due_on = sonrakiVade(r.rrule, r.next_due_on); save(); return delay(r.next_due_on); },
    },
    plans: {
      ...
      async addItem(i) { const db = load(); const row = { id: uid('pi'), household_id: db.household.id, kind: 'checklist', is_done: false, ...i }; masrafEsitle(db, row); db.plan_items.push(row); save(); return delay(row); },
      async updateItem(id, patch) { const db = load(); const r = db.plan_items.find((x) => x.id === id); Object.assign(r, patch); masrafEsitle(db, r); save(); return delay(r); },
      async removeItem(id) {
        const db = load(); const r = db.plan_items.find((x) => x.id === id);
        if (r?.transaction_id) db.transactions = db.transactions.filter((t) => t.id !== r.transaction_id);
        db.plan_items = db.plan_items.filter((x) => x.id !== id); save(); return delay();
      },
      // Plan silinince kalemleri gider; ödenmiş harcamalar DURUR (0028).
      async remove(id) {
        const db = load();
        db.plans = db.plans.filter((p) => p.id !== id);
        db.plan_items = db.plan_items.filter((i) => i.plan_id !== id);
        for (const t of db.transactions) if (t.plan_id === id) t.plan_id = null;
        save(); return delay();
      },
    ...
    shopping: {
      ...
      async addItem(i) {
        ...
        if (catalog_key) {
          ...
          // "Bitmiş olabilir" için gün günlüğü (0024'teki tetiğin aynası).
          db.shopping_add_log ||= [];
          if (!db.shopping_add_log.some((r) => r.catalog_key === catalog_key && r.added_on === today())) {
            db.shopping_add_log.push({ catalog_key, added_on: today() });
          }
        }
        save(); return delay(row);
      },
    ...
    // Demo: fiş cihazda küçültülmüş resim olarak tutulur (en fazla 5).
    receipts: {
      async url(path) { return delay((load().fisler || {})[path] || null); },
      async attach(txnId, file, oldPath) {
        const db = load(); db.fisler ||= {};
        if (!oldPath && Object.keys(db.fisler).length >= 5) throw new Error('demo-fis-full');
        const { kucult } = await import('../album.js');
        const { blob } = await kucult(file, 1000, 0.7);
        const url = await new Promise((res, rej) => { const r = new FileReader(); r.onload = () => res(r.result); r.onerror = rej; r.readAsDataURL(blob); });
        const path = `${db.household.id}/${txnId}-${uid('f')}.jpg`;
        if (oldPath) delete db.fisler[oldPath];
        db.fisler[path] = url;
        const t = db.transactions.find((x) => x.id === txnId); if (t) t.receipt_path = path;
        save(); return delay(path);
      },
      async remove(txnId, path) {
        const db = load(); if (db.fisler) delete db.fisler[path];
        const t = db.transactions.find((x) => x.id === txnId); if (t) t.receipt_path = null;
        save(); return delay();
      },
    },
    notifications: {
    ...
    summary: {
      ...
      // 0027'nin aynası: harcama varsayılan AİLENİN; for_member_id doluysa
      // o kişinin kişisel harcaması. Aile özeti kişiseli saymaz.
      async month(period) {
        const db = load();
        const rows = db.transactions.filter((t) => periodOf(t.occurred_on) === period);
        const gider = rows.filter((t) => t.kind === 'expense');
        const aile = gider.filter((t) => !t.for_member_id);
        const benim = gider.filter((t) => t.for_member_id && t.for_member_id === db.me?.id);
        const topla = (xs) => xs.reduce((s, t) => s + t.amount_base, 0);
        const kategoriler = (xs) => {
          const byCat = {};
          for (const t of xs) {
        ...
        const fixed = aile.reduce((s, t) => {
          const leaf = db.categories.find((x) => x.id === t.category_id);
          const c = rollupCategory(db, t.category_id);
          return s + (leaf?.is_fixed || c?.is_fixed ? t.amount_base : 0);
        }, 0);
        const expense = topla(aile);
        return delay({
          period, expense, total_expense: topla(gider),
          income: topla(rows.filter((t) => t.kind === 'income')),
          fixed, variable: expense - fixed,
          by_category: kategoriler(aile),
          personal: { total: topla(benim), limit: db.personal_limit || null, by_category: kategoriler(benim) },
        });
      },
      async budgetStatus(period) {
        const db = load();
        const m = await this.month(period);
        // 0029: her kategori için dönemi <= bu ay olan en son ayar geçerli; 0 = kaldırıldı.
        const gecerli = new Map();
        for (const b of [...db.budgets].filter((x) => x.period <= period).sort((x, y) => x.period.localeCompare(y.period))) {
          gecerli.set(b.category_id || null, b);
        }
        return [...gecerli.values()].filter((b) => Number(b.amount_base) > 0).map((b) => {
          ...
          return { category_id: b.category_id, category_name: c?.name || 'Toplam', icon: c?.icon || '🎯', budget: b.amount_base, spent, remaining: b.amount_base - spent, pct: b.amount_base ? Math.round((spent / b.amount_base) * 1000) / 10 : 0, from_period: b.period };
       ...
       */
      async trend(period, months = 6, scope = 'family') {
        const db = load();
        ...
        // 0027: 'family' = aile gideri + gelir; 'personal' = yalnızca benim kişisel giderim.
        const kapsam = scope === 'personal'
          ? (t) => t.kind === 'expense' && t.for_member_id && t.for_member_id === db.me?.id
          : (t) => t.kind === 'income' || (t.kind === 'expense' && !t.for_member_id);
        const rows = db.transactions.filter(icinde).filter(kapsam);
      ...
      async agenda(from, days) {
        ...
        return delay({
          ...
          bills: db.recurring.filter((r) => r.is_active && r.next_due_on <= to && (r.next_due_on >= from || !r.auto_post)).sort((a, b) => a.next_due_on.localeCompare(b.next_due_on)),
    ...
    personal: {
      _db() {
        ...
        db.supplements ||= []; db.supplement_logs ||= [];
        return db;
      },
      ...
      async updateBlock(id, patch) {
        const db = this._db(); const b = db.day_blocks.find((x) => x.id === id);
        if (b) Object.assign(b, patch); save(); return delay(b);
      },
      ...
      // TAKVİYELER (0022) — SQL'deki kuralların aynası: tavan per_day,
      // sıfıra inince kayıt silinir, oran alınan doz / alınması gereken doz.
      async supplements(date) {
        const db = this._db(); const d = date || today();
        const items = db.supplements.filter((s) => s.is_active !== false).map((s) => {
          const taken = db.supplement_logs.find((l) => l.supplement_id === s.id && l.on_date === d)?.taken || 0;
          return { ...s, taken, done: taken >= (s.per_day || 1) };
        });
        const hedef = db.supplements.filter((s) => s.is_active !== false)
          .reduce((t, s) => t + (s.per_day || 1), 0) * 7;
        let alinan = 0;
        for (let i = 6; i >= 0; i -= 1) {
          const g = addDays(d, -i);
          alinan += db.supplement_logs.filter((l) => l.on_date === g
            && db.supplements.some((s) => s.id === l.supplement_id && s.is_active !== false))
            .reduce((t, l) => t + l.taken, 0);
        }
        return delay({ date: d, items, adherence7: hedef ? Math.round((100 * alinan) / hedef) : null });
      },
      async takeSupplement(id, delta, date) {
        const db = this._db(); const d = date || today();
        const s = db.supplements.find((x) => x.id === id);
        if (!s) return delay(0);
        const i = db.supplement_logs.findIndex((l) => l.supplement_id === id && l.on_date === d);
        const su = i >= 0 ? db.supplement_logs[i].taken : 0;
        const yeni = Math.min(s.per_day || 1, Math.max(0, su + (delta ?? 1)));
        if (yeni <= 0) { if (i >= 0) db.supplement_logs.splice(i, 1); save(); return delay(0); }
        if (i >= 0) db.supplement_logs[i].taken = yeni;
        else db.supplement_logs.push({ supplement_id: id, on_date: d, taken: yeni });
        save(); return delay(yeni);
      },
      async addSupplement(s) {
        const db = this._db();
        const row = { id: uid('sup'), is_active: true, per_day: 1, ...s };
        db.supplements.push(row); save(); return delay(row);
      },
    ...
    /**
     * AİLE ALBÜMÜ — demo: küçültülmüş fotoğraf data: adresi olarak cihazda
     * duruyor. Tarayıcı deposu ~5 MB; bu yüzden demo 6 fotoğrafla sınırlı ve
     * daha küçük (900 px) saklanıyor. Gerçek kipte sınır yok.
     */
    album: {
      async photoOfDay(date) {
        const list = load().album || [];
        const i = gununSirasi(date || today(), list.length);
        if (i < 0) return delay({ count: 0 });
        return delay({ ...list[i], count: list.length });
      },
      async list() { return delay([...(load().album || [])]); },
      async upload(file) {
        const db = load(); db.album ||= [];
        if (db.album.length >= 6) throw new Error('demo-album-full');
        const { kucult } = await import('../album.js');
        const { blob, width, height } = await kucult(file, 900, 0.78);
        const url = await new Promise((res, rej) => {
          const fr = new FileReader(); fr.onload = () => res(fr.result); fr.onerror = rej; fr.readAsDataURL(blob);
        });
        const row = { id: uid('ph'), url, width, height, created_at: new Date().toISOString() };
        db.album.push(row); save(); return delay(row);
      },
      async remove(id) { const db = load(); db.album = (db.album || []).filter((p) => p.id !== id); save(); return delay(); },
    },
    /** Demo kipinde sunucu yok, bildirim de yok; tercihler cihazda tutuluyor. */
    push: {
      demo: true,
      async vapidKey() { throw new Error('demo'); },
      async subscribe() { return delay(); },
      async unsubscribe() { return delay(); },
      async prefs() {
        const p = load().notify_prefs || {};
        return delay({ events: true, event_lead_min: 30, plans: true, blocks: true, supplements: true,
          supplements_at: '09:00', goals: true, goals_at: '20:30', digest: true, weekly: true, weekly_at: '19:00', occasions: true, bills: true, monthly: true, ...p, devices: 0 });
      },
      async setPrefs(p) { const db = load(); db.notify_prefs = { ...(db.notify_prefs || {}), ...p }; save(); return delay(); },
      async test() { return delay(0); },
      async weeklyPreview() { return delay(null); },
      async monthlyPreview() { return delay(null); },
    },
    suggest: {
      async merchants() {
        const db = load();
        return delay(magazaOgren(db.transactions.filter((t) => !(t.for_member_id && t.for_member_id !== db.me?.id))));
      },
      async expenses(date) {
        const db = load();
        const rows = harcamaOnerileri(db.transactions, date || today(), 4).map((r) => {
          const c = db.categories.find((x) => x.id === r.category_id);
          return { ...r, category_name: c?.name || null, category_icon: c?.icon || null };
        });
        return delay(rows);
      },
      async restock(date) { return delay(bitmisOlabilir(load().shopping_add_log || [], date || today(), 6)); },
    },
