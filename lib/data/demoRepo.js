...
import { expandRRule, today, addDays, periodOf, nextOccasionDate, daysBetween } from '../dates.js';
...
export function createDemoRepo() {
  return {
    ...
    plans: {
      ...
      async contributions(planId) { return delay(load().goal_contributions.filter((g) => g.plan_id === planId)); },
    },
    occasions: {
      async list() {
        const db = load();
    ...
    tasks: {
      ...
      async toggle(id) { const db = load(); const r = db.tasks.find((x) => x.id === id); r.is_done = !r.is_done; r.done_at = r.is_done ? new Date().toISOString() : null; save(); return delay(r); },
    },
    shopping: {
      ...
      async addItem(i) { const db = load(); const row = { id: uid('si'), household_id: db.household.id, list_id: db.shopping_lists[0].id, is_checked: false, added_by_member_id: db.me?.id, ...i }; db.shopping_items.push(row); save(); return delay(row); },
    ...
    notifications: {
      async list() { return delay(load().notifications.filter((n) => !n.read_at)); },
    ...
    auth: {
      async signInWithEmail() { return delay(); },
