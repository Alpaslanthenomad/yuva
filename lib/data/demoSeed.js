...
export function buildDemo(locale = 'tr') {
  ...
  const transactions = [
    ...
    tx(addDays(mStart, 4), 'expense', 'a_bci', 420000, 'CLP', 'c_okul', x.tx.okulTaksit),
    ...
    tx(d(5), 'expense', 'a_kart', 45000, 'CLP', 'c_kurs', x.tx.yuzme),
    ...
    tx(d(1), 'expense', 'a_nakit', 6500, 'CLP', 'c_diger', x.tx.kirtasiye),
    tx(T, 'expense', 'a_kart', 23400, 'CLP', 'c_saglik', x.tx.eczane, { for_member_id: 'm_baba' }),
    // Eşin kişisel harcaması: listede yalnızca tutarı görünür (0029).
    tx(d(1), 'expense', 'a_kart', 29900, 'CLP', 'c_giyim', x.tx.kislik, { for_member_id: 'm_anne', paid_by_member_id: 'm_anne' }),
    ...
    tx(addDays(lmStart, 20), 'expense', 'a_kart', 129000, 'CLP', 'c_giyim', x.tx.kislik),
  ];
  ...
  return {
    ...
    task_completions: [],
    goal_contributions: [{ id: uid('g'), plan_id: 'p_araba', household_id: hid, amount: 5400, currency: 'USD', fx_rate: rates.USDCLP, amount_base: toBase(5400, 'USD'), on_date: T }],
  };
}
