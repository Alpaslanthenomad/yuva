...
/**
 * Grafik ekseni için kısa dönem etiketi: "Eyl" / "Sep".
 * Yıl YALNIZCA ocakta yazılır ("Oca 26"): altı çubuğun altına altı kere yıl
 * yazmak telefon genişliğinde okunmuyor, ama pencere yıl atlıyorsa bunun
 * görünmesi gerekiyor — ocak tam o sınır.
 */
export function fmtPeriodShort(period, loc) {
  const s = S(loc), [y, m] = period.split('-').map(Number);
  const ad = s.months[m - 1].slice(0, 3);
  return m === 1 ? `${ad} ${String(y).slice(2)}` : ad;
}
...
export function expandRRule(rrule, startIso, rangeFrom, rangeTo, exdates = []) {
  ...
  let cursor = startIso;
  let guard = 0;
  // Ayın günü: BYMONTHDAY varsa ondan, yoksa başlangıç tarihinden.
  // Neden gerekli: tekrarlayan görev her tamamlanışta yeni vadesinden devam
  // ediyor. 31 Ocak → 28 Şubat'a kırpılınca çapa 28'e düşüyor ve görev bir
  // daha asla 31'e dönmüyordu (28 Mart, 28 Nisan...). BYMONTHDAY çapayı
  // kuralda tutar; -1 "ayın son günü" demektir.
  const bymonthday = parts.BYMONTHDAY ? Number(parts.BYMONTHDAY) : null;
  const startDom = bymonthday && bymonthday > 0 ? bymonthday : fromISODate(startIso).getDate();
  ...
  while (cursor <= rangeTo && produced < count && guard++ < 5000) {
    ...
    // BYDAY yoksa olay kendi gününde tekrar eder; hafta başına çekilmemeli.
      ...
      d.setDate(bymonthday === -1 ? dim : Math.min(startDom, dim)); cursor = toISODate(d);
    } else if (freq === 'YEARLY') {
...
/**
 * Bir tarih, plan aralığının kaçıncı günü? Aralık dışındaysa null.
 * Plan (seyahat, etkinlik) takvimde bant olarak gösterilir; gün numarası
 * "2. gün / 5" biçiminde yazılır.
 * @returns {{day:number, total:number}|null}
 */
export function dayInRange(iso, startIso, endIso) {
  if (!startIso) return null;
  const end = endIso || startIso;
  if (iso < startIso || iso > end) return null;
  return { day: daysBetween(startIso, iso) + 1, total: daysBetween(startIso, end) + 1 };
}

/**
 * Tekrar seçeneği anahtarı → RRULE.
 * Formlarda beş seçenek var: none/daily/weekly/monthly/yearly.
 *
 * Aylıkta çapa kurala yazılır (BYMONTHDAY). Neden: tekrarlayan GÖREV her
 * tamamlanışta yeni vadesinden devam ediyor. Çapa kuralda değilse 31 Ocak →
 * 28 Şubat'a kırpılınca seri 28'e düşüyor ve bir daha 31'e dönmüyor.
 * Olaylarda seri hep başlangıç tarihinden açıldığı için sorun çıkmıyordu;
 * yine de aynı kural kullanılıyor — tek davranış, tek yer.
 *
 * @param {string} key none|daily|weekly|monthly|yearly
 * @param {string|null} anchorIso çapa tarihi (görevin vadesi / olayın başlangıcı)
 * @returns {string|null} RRULE; tekrar yoksa null
 */
export function buildRRule(key, anchorIso) {
  if (!key || key === 'none') return null;
  if (key === 'daily') return 'FREQ=DAILY';
  if (key === 'weekly') return 'FREQ=WEEKLY';
  if (key === 'yearly') return 'FREQ=YEARLY';
  if (key === 'monthly') {
    if (!anchorIso) return 'FREQ=MONTHLY';
    return `FREQ=MONTHLY;BYMONTHDAY=${fromISODate(anchorIso).getDate()}`;
  }
  return null;
}

/**
 * RRULE → tekrar seçeneği anahtarı. Yalnızca FREQ'e bakar; BYMONTHDAY gibi
 * ek parçalar seçeneği değiştirmez (düzenlemede seçim doğru görünsün diye).
 * Tanımadığı kural 'none' değil, FREQ'ine en yakın seçenek olur; hiç FREQ
 * yoksa 'none'.
 */
export function freqKeyOf(rrule) {
  if (!rrule) return 'none';
  const m = /FREQ=([A-Z]+)/.exec(rrule);
  const k = { DAILY: 'daily', WEEKLY: 'weekly', MONTHLY: 'monthly', YEARLY: 'yearly' }[m?.[1]];
  return k || 'none';
}
