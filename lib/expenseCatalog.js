...
// ---- Düzenli ödemeler için hazır seçimler (0033) --------------------------
// Dokununca ad, kategori ve sıklık dolar; tutar ve vade kullanıcıdan.
// Kategori eşleşmezse boş kalır (presetCategoryId).
export const DUZENLI_ONERILER = [
  { key: 'kira',     emoji: '🔑', tr: 'Kira',              es: 'Arriendo',          freq: 'MONTHLY', cat: /kira|arriendo|hipoteca|konut/i,        parentCat: /^ev$|hogar|casa/i },
  { key: 'aidat',    emoji: '🏢', tr: 'Aidat',             es: 'Gastos comunes',    freq: 'MONTHLY', cat: /aidat|gastos comunes/i,                 parentCat: /^ev$|hogar|casa/i },
  { key: 'elektrik', emoji: '💡', tr: 'Elektrik',          es: 'Luz',               freq: 'MONTHLY', cat: /elektrik|luz|electricidad/i,            parentCat: /^ev$|hogar|casa/i },
  { key: 'su',       emoji: '🚰', tr: 'Su',                es: 'Agua',              freq: 'MONTHLY', cat: /^su$|agua/i,                            parentCat: /^ev$|hogar|casa/i },
  { key: 'gaz',      emoji: '🔥', tr: 'Gaz',               es: 'Gas',               freq: 'MONTHLY', cat: /^gaz$|^gas$/i,                          parentCat: /^ev$|hogar|casa/i },
  { key: 'internet', emoji: '📶', tr: 'İnternet',          es: 'Internet',          freq: 'MONTHLY', cat: /internet/i,                             parentCat: /^ev$|hogar|casa/i },
  { key: 'telefon',  emoji: '📱', tr: 'Telefon',           es: 'Celular',           freq: 'MONTHLY', cat: /telefon|celular|internet/i,             parentCat: /^ev$|hogar|casa/i },
  { key: 'sigorta',  emoji: '🛡️', tr: 'Sağlık sigortası',  es: 'Isapre / seguro',   freq: 'MONTHLY', cat: /sağlık sigortası|isapre|seguro de salud/i, parentCat: /sağlık|salud/i },
  { key: 'okul',     emoji: '🎒', tr: 'Okul taksidi',      es: 'Colegio',           freq: 'MONTHLY', cat: /^okul$|colegio/i,                       parentCat: /çocuk|niños|hijos/i },
  { key: 'arac',     emoji: '🚗', tr: 'Araç sigortası',    es: 'Seguro auto',       freq: 'MONTHLY', cat: /araç|seguro auto|vehículo/i,            parentCat: /ulaşım|transporte/i },
  { key: 'netflix',  emoji: '📺', tr: 'Netflix',           es: 'Netflix',           freq: 'MONTHLY', cat: /abonelik|suscrip/i,                     parentCat: /eğlence|entreten|ocio/i },
  { key: 'spotify',  emoji: '🎵', tr: 'Spotify',           es: 'Spotify',           freq: 'MONTHLY', cat: /abonelik|suscrip/i,                     parentCat: /eğlence|entreten|ocio/i },
  { key: 'kredi',    emoji: '🏦', tr: 'Kredi taksidi',     es: 'Cuota crédito',     freq: 'MONTHLY', cat: /kredi|crédito/i,                        parentCat: /vergi|diğer|otros/i },
];

/** Vade sonrası bir sonraki tarih — SQL'deki recurring_next'in aynası. */
export function sonrakiVade(rrule, iso) {
  const r = String(rrule || '').toUpperCase();
  const d = new Date(iso + 'T12:00:00');
  if (r.includes('FREQ=WEEKLY')) d.setDate(d.getDate() + 7);
  else if (r.includes('FREQ=YEARLY')) d.setFullYear(d.getFullYear() + 1);
  else if (r.includes('FREQ=DAILY')) d.setDate(d.getDate() + 1);
  else {
    // Ay sonu taşması: 31 Ocak + 1 ay = 28/29 Şubat (Postgres ile aynı).
    const gun = d.getDate(); d.setDate(1); d.setMonth(d.getMonth() + 1);
    const son = new Date(d.getFullYear(), d.getMonth() + 1, 0).getDate();
    d.setDate(Math.min(gun, son));
  }
  return `${d.getFullYear()}-${String(d.getMonth() + 1).padStart(2, '0')}-${String(d.getDate()).padStart(2, '0')}`;
}

// Şili'de sık görülen markalar → hazır seçimdeki karşılığı (kategori için).
const MARKALAR = {
  copec: 'fuel', shell: 'fuel', petrobras: 'fuel', aramco: 'fuel', enex: 'fuel',
  cabify: 'uber', didi: 'uber', 'uber eats': 'delivery', rappi: 'delivery', pedidosya: 'delivery', 'pedidos ya': 'delivery',
  starbucks: 'cafe', 'juan valdez': 'cafe', dunkin: 'cafe',
  'cruz verde': 'pharmacy', salcobrand: 'pharmacy', ahumada: 'pharmacy', 'dr simi': 'pharmacy',
  tottus: 'lider', 'acuenta': 'lider', 'a cuenta': 'lider', walmart: 'lider', 'lider express': 'lider', 'express de lider': 'lider',
  oxxo: 'minimarket', ok: 'minimarket', 'ok market': 'minimarket', 'upa!': 'minimarket',
};

/**
 * Yazılan yer adı hazır listedeki bir yerse (Copec, Uber, Jumbo…) onu döndür.
 * Geçmişte hiç girilmemiş bir yer için bile kategori tahmini yapılabilsin.
 */
export function katalogdanBul(ad) {
  const k = (x) => String(x || '').normalize('NFD').replace(/[\u0300-\u036f]/g, '').toLowerCase().replace(/\s+/g, ' ').trim();
  const a = k(ad);
  if (!a) return null;
  const takma = MARKALAR[a];
  return EXPENSE_PRESETS.find((p) => (takma ? p.key === takma : [p.key, p.tr, p.es].some((x) => k(x) === a))) || null;
}
