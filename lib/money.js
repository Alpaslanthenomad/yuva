...
/**
 * Plan toplamları.
 * Harcama YALNIZCA gider işlemlerinden gelir (gelir/transfer sayılmaz) ve
 * işlem tarihinde dondurulmuş `amount_base` kullanılır — güncel kurla yeniden
 * hesaplanmaz. Plan kendi para biriminde gösteriliyorsa çevrim tek seferde,
 * en sonda yapılır.
 * @param {Object[]} txns plana bağlı işlemler
 * @param {Object[]} contribs hedef planına yapılan katkılar
 * @param {string} baseCurrency hanenin ana para birimi
 * @param {string} targetCurrency planın gösterim para birimi
 * @param {Object} rates kur tablosu
 * @returns {{spent:number, contributed:number}}
 */
export function planTotals(txns = [], contribs = [], baseCurrency = 'CLP', targetCurrency = baseCurrency, rates = {}) {
  const sumBase = (rows) => rows.reduce((s, r) => s + Number(r.amount_base ?? 0), 0);
  const toTarget = (v) => (targetCurrency === baseCurrency ? v : (convert(v, baseCurrency, targetCurrency, rates) ?? 0));
  return {
    spent: toTarget(sumBase(txns.filter((x) => x.kind === 'expense'))),
    contributed: toTarget(sumBase(contribs)),
  };
}
